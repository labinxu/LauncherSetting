...
public class CellLayout extends ViewGroup {
    ...
    private static int mAppIconBGColor = 0X00000000;
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mWorkspaceCellGap = res.getDimensionPixelSize(R.dimen.workspace_cell_gap);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (DEBUG_VISUALIZE_OCCUPIED) {
            ...
            //jeremy
            // add by labin.xu
            //int color = APP_ICON_BG[3];
            int color = mAppIconBGColor;
            ColorDrawable cd = new ColorDrawable(color);
            //int index = new Random(System.currentTimeMillis()).nextInt(6);
            //ColorDrawable cd = new ColorDrawable(APP_ICON_BG[index]);
            ...
            for (int i = 0; i < mCountX; i++) {
                for (int j = 0; j < mCountY; j++) {
                    if (mOccupied[i][j]) {
                        if (mIsHotseat) {
                            if (i == 0) {
                                cd.setBounds(0, 0, mCellWidth - mWorkspaceCellGap, mCellHeight);
                            } else {
                                cd.setBounds(0, 0, mCellWidth, mCellHeight);
                            }
                        } else {
                            if (i == 0 && j != 0) {
                                if (j == 2) {
                                    ...
                                    cd.setBounds(0, 0, mCellWidth - mWorkspaceCellGap, mCellHeight - mWorkspaceCellGap);
                                }
                            } else {
                                if (j == 2) {
                                    cd.setBounds(0, 0, mCellWidth, mCellHeight);
                                } else {
                                    cd.setBounds(0, 0, mCellWidth, mCellHeight - mWorkspaceCellGap);
                                }
                            }
                        }
    ...
    // add by labin.xu
    public static void setAppBGColor(int color)
    {
        mAppIconBGColor = color;
    }
    public static int getmAppIconBGColor()
    {
        return mAppIconBGColor;
    }
    // end
    @Override
